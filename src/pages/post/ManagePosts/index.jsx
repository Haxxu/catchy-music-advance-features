...
import { deletePostByIdUrl, getUserPostsByUserIdUrl } from '~/api/urls/postUrl';
...
const ManagePosts = () => {
    ...
    const debouncedValue = useDebounce(searchPost, 600);
    const { userId } = useAuth();
    const { t } = useTranslation();

    useEffect(() => {
        const fetchData = async () => {
            const { data } = await axiosInstance(getUserPostsByUserIdUrl(userId), {
    ...
    return (
        <div className={cx('container')}>
            <div className={cx('header')}>
                <h1>{t('My Posts')}</h1>
                <Link to={routes.managePosts_newPost}>
                    <Button
                        size='large'
                        color='secondary'
                        variant='contained'
            ...
            <div className={cx('input-container')}>
                ...
                <input
                    type='text'
                    placeholder={t('Search for my post')}
                    value={searchPost}
